...
def test_method(themethod, quiet=False):
    ...
    if not re.search(":calls:", inspect.getdoc(themethod)):
        return True
    if not re.search("<\S*>", inspect.getdoc(themethod)):
        return True

    method_string = inspect.getfile(themethod) + " " + themethod.__name__
    call_lines = re.findall("`(POST|GET|PUT|PATCH|DELETE)([^<]*)<([^>]*)>`_",
...
def test_docstring(method_string, call_line, quiet):
    ...
    if html_doc_response.status_code != requests.codes.ok:
        if not quiet:
            print "%s docstring URL request returned %d" % \
                    (method_string, html_doc_response.status_code)
        return False

    endpoint_h2 = re.search("name=[\'\"]%s[\'\"]" % endpoint_name,
                            html_doc_response.text)
    if not endpoint_name:
        if not quiet:
            print "%s docstring URL does not contain an endpoint name in link \
             to API documentation" % method_string
        return False
    if not endpoint_h2:
        if not quiet:
            print "%s docstring refers to %s in %s, not found" \
                    % (method_string, endpoint_name, docfile_URL)
        return False

    endpoint_element_re = \
        re.compile('<h3 class=[\"\']endpoint[\"\']>[^<]*<\/h3>')
    endpoint_search_start_pos = \
        endpoint_element_re.search(html_doc_response.text,
                                   endpoint_h2.end()).start()
    ...
    if not endpoint_element_list:
        if not quiet:
            print "Found no endpoint after %s in %s" % \
                (endpoint_name, docfile_URL)
        return False
    docfile_lines = []
